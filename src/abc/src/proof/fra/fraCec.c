...
int Fra_FraigSat( Aig_Man_t * pMan, ABC_INT64_T nConfLimit, ABC_INT64_T nInsLimit, int nLearnedStart, int nLearnedDelta, int nLearnedPerce, int fFlipBits, int fAndOuts, int fNewSolver, int fVerbose )
{
    ...
    //    pCnf = Cnf_DeriveSimple( pMan, Aig_ManCoNum(pMan) );
        ...
        if ( fAndOuts )
        {
            ...
            if ( !Cnf_DataWriteAndClauses( pSat, pCnf ) )
            {
                ...
                Cnf_DataFree( pCnf );
                // printf ( "return after Cnf_DataWriteAndClauses( pSat, pCnf )" );
                return 1;
            }
        }
        else
        {
            ...
            if ( !Cnf_DataWriteOrClause2( pSat, pCnf ) )
            {
                sat_solver2_delete( pSat );
                Cnf_DataFree( pCnf );
                // printf ( "Cnf_DataWriteAndClauses( pSat, pCnf )" );
                return 1;
            }
        }
    ...
    //    pCnf = Cnf_DeriveSimple( pMan, Aig_ManCoNum(pMan) );
        ...
        if ( pSat == NULL )
        {
            printf( "Return because pSat = NULL" );
            Cnf_DataFree( pCnf );
            return 1;
        }
        ...
        else
        {
            ...
            {
                // printf( "Return after Cnf_DataWriteOrClauses( pSat, pCnf )" );
                sat_solver_delete( pSat );
                Cnf_DataFree( pCnf );
                return 1;
            }
    ...
    //    ABC_PRT( "Time", Abc_Clock() - clk );
        ...
        // simplify the problem
        clk = Abc_Clock();
        status = sat_solver_simplify(pSat); // TODO
...
//            pSat->verbosity = 1;
        ...
        // printf("after line 256, status = %d\n",status);
        if ( status == l_Undef )
        {
           printf( "The problem timed out.\n" );
            RetValue = -1;
...
int Fra_FraigCec( Aig_Man_t ** ppAig, int nConfLimit, int fVerbose )
{
    ...
    clk = Abc_Clock(); //TODO
    RetValue = Fra_FraigSat( pAig, (ABC_INT64_T)2*nBTLimitStart, (ABC_INT64_T)0, 0, 0, 0, 1, 0, 0, fVerbose );
    if ( fVerbose )
    {
...
//printf( "Running fraiging with %d BTnode and %d BTmiter.\n", pParams->nBTLimitNode, pParams->nBTLimitMiter );
        ...
        if ( fVerbose )
        {
            printf( "Fraiging (i=%d):   Nodes = %6d.  \n", i+1, Aig_ManNodeNum(pAig) );
            ABC_PRT( "Time", Abc_Clock() - clk );
        }

        // check the miter status
        RetValue = Fra_FraigMiterStatus( pAig );
        // printf( "Line404. Miterstatus, Retvalue = %d \n", RetValue) ;
        if ( RetValue >= 0 )
            break;
        // printf( "fraCec:402 \n" );
        // perform rewriting
        ...
        RetValue = Fra_FraigMiterStatus( pAig );
        if ( RetValue >= 0 ){
            // printf( "Line:421. Miterstatus, Retvalue = %d \n", RetValue) ;
            break;
        }
        // try simulation
...
/**Function*************************************************************
  ...
  SeeAlso     [Fra_FraigCec]

***********************************************************************/
int Eda_FraigCec( Aig_Man_t ** ppAig, int nConfLimit, int fVerbose, char * pFileName )
{
    int nBTLimitStart =       3000;   //300;   // starting SAT run
    int nBTLimitFirst =          3;   //2;   // first fraiging iteration
    int nBTLimitLast  = nConfLimit;   // the last-gasp SAT run
