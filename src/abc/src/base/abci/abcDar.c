...
/**Function*************************************************************
  ...
  Synopsis    [Write out "EQ" with file name pFilename]

  Description []
               
  SideEffects [self modified]

  SeeAlso     []

***********************************************************************/
void Eda_WriteOutEQ(char* pFileName){
    FILE* pFile;
...
/**Function*************************************************************
  ...
  Synopsis    [Write out "NEQ" with file name pFilename]
...
***********************************************************************/
void Eda_WriteOutNEQ(char* pFileName, Abc_Ntk_t* pNtk){
    FILE* pFile;
    pFile = fopen(pFileName, "w");
    if(pFile!=NULL){
        int i;
        int * pOut = pNtk->pModel;
        Abc_Obj_t * pNode;
        fputs("NEQ", pFile);
        Abc_NtkForEachPi( pNtk, pNode, i ){
            fprintf(pFile, "\n%s %d", Abc_ObjName(pNode), pOut[i]);
        }
        fclose(pFile);
    }
}
...
/**Function*************************************************************
  ...
  Description [with addtional argument pFileName]
               
  SideEffects [self modified]

  SeeAlso     [Abc_NtkDarCec]

***********************************************************************/
int Eda_NtkDarCec( Abc_Ntk_t * pNtk1, Abc_Ntk_t * pNtk2, int nConfLimit, int fPartition, int fVerbose, char* pFileName )
{
    Aig_Man_t * pMan, * pMan1, * pMan2;
    Abc_Ntk_t * pMiter;
    int RetValue;
    ...
    if ( RetValue == 0 )
    {
        ...
        if ( pNtk2 == NULL ){
            pNtk1->pModel = Abc_NtkVerifyGetCleanModel( pNtk1, 1 );
            Eda_WriteOutNEQ( pFileName, pNtk2 );
        }            
        else{
            pNtk2->pModel = Abc_NtkVerifyGetCleanModel( pNtk2, 1 );
            Eda_WriteOutNEQ( pFileName, pNtk2 );
        }
        Abc_NtkDelete( pMiter );
        return 0;
    ...
    // perform verification
    RetValue = Eda_FraigCec( &pMan, 10000000, fVerbose, pFileName ); //TODO: 100000
    // transfer model if given
    if ( pNtk2 == NULL )
        pNtk1->pModel = (int *)pMan->pData, pMan->pData = NULL;
    else
        pNtk2->pModel = (int *)pMan->pData, pMan->pData = NULL;
...
finish:
    ...
    {
        Eda_WriteOutEQ( pFileName );
        Abc_Print( 1, "Networks are equivalent, proven in FriagCEC.     " );
ABC_PRT( "Time", Abc_Clock() - clkTotal );
    }
    else if ( RetValue == 0 )
    {
        Eda_WriteOutNEQ( pFileName, pNtk2==NULL?pNtk1:pNtk2 );
        Abc_Print( 1, "Networks are NOT EQUIVALENT, proven in FriagCEC.     " );
ABC_PRT( "Time", Abc_Clock() - clkTotal );
    }
    else
    {
