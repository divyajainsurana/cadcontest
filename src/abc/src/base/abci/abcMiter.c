...
void Abc_NtkMiterFinalize( Abc_Ntk_t * pNtk1, Abc_Ntk_t * pNtk2, Abc_Ntk_t * pNtkMiter, int fComb, int nPartSize, int fImplic, int fMulti )
{
    ...
    if ( fComb )
    {
        ...
        //     }
        // }
        int com = Abc_NtkPoNum(pNtk1);
        for (i = 0; i < com ; i+=2 ){
            pNode = Abc_NtkPo( pNtk1, i );
            Vec_PtrPush( vPairs, Abc_ObjChild0Copy(pNode) );
            pNode = Abc_NtkPo( pNtk1, i+1 );
            Vec_PtrPush( vPairs, Abc_ObjChild0Copy(pNode) );
            pNode = Abc_NtkPo( pNtk2, i );
            Vec_PtrPush( vPairs, Abc_ObjChild0Copy(pNode) );
            pNode = Abc_NtkPo( pNtk2, i+1 );
            Vec_PtrPush( vPairs, Abc_ObjChild0Copy(pNode) );
        }
    }
    // while calling by dcec, fComb = 0
    else
    {
    ...
    // add the miter
    // nPartSize = 0 while calling by dcec
    if ( nPartSize <= 0 )
    {
        if ( !fMulti )
        {
